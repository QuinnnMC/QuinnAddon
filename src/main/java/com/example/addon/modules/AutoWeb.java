...
private final Setting<List<Block>> blocks = sgGeneral.add(
    new BlockListSetting.Builder()
        .name("blocks")
        .description("Blocks AutoWeb can place.")
...
private final Setting<Boolean> targetFeet = sgTarget.add(
    new BoolSetting.Builder()
        .name("target-feet")
        .description("Place the web at the target's feet.")
        .defaultValue(true)
        .build()
);
...
public AutoWeb() {
    super(
        QuinnAddon.CATEGORY,
        "auto-web",
        "Automatically places cobwebs around nearby enemy players."
    );
}
...
private void onTick(TickEvent.Pre event) {
    ...
    FindItemResult web = InvUtils.findInHotbar(stack -> {
...
private void onRender(Render3DEvent event) {
    ...
    long elapsed = System.currentTimeMillis() - lastPlacedRenderTime;
...
private BlockPos getPredictedPosition(PlayerEntity target) {
    ...
    return BlockPos.ofFloored(x, y + 1, z);
}
